...
class Bookmark(models.Model):
    ...
    # 작성일 -> 서버 시간을 읽어서 timestamp값을 만들어서 추가
    # 자동 옵션 auto_now(업데이트 할 때마다 시간 갱신), auto_now_add(첫 작성 때만 시간 추가)
    ...
    # 메타 클래스는 옵션 클래스 - 내가 상속을 받았는데, 속성값에 변경이 필요하다면 메타 클래스 사용
    # DB 변경사항이 있으므로 DB에 적용: makemigrations, migrate
    # python manage.py makemigrations bookmark
